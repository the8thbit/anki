...
class Browser(QMainWindow):
    ...
    def setupMenus(self):
        ...
        f.actionPostpone_reviews.triggered.connect(self.onPostpone_reviews)
        # jumps
...
update cards set usn=?, mod=?, did=? where id in """ + scids,
    ...
    def onPostpone_reviews(self):
        cids=self.selectedCards()
        self.col.addDelay(cids, self.mw.pm.profile)
    ...
    def ___onBatchEdit(self, qdialog, frm, pos, nids, fields):
        ...
        for nid in nids:
            ...
            if self.mw.pm.profile.get("newLineInBatchEdit", False):
                spacer = ""
            if pos == "after":
...
class ChangeModel(QDialog):
    ...
    def _rebuildMap(self, key=None):
        ...
        if self.browser.mw.pm.profile.get("preserveName", True):
